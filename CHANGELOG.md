...
### Security
- **Depth-guard HTML-layout против deep-DOM StackOverflow** (Вектор 3, PR-3c) — **исправление реального
  DoS**. Взаимно-рекурсивный обход DOM в общем `LayoutEngine` (`Foliant.Rendering.Html`:
  `LayoutChildren`/`LayoutBlock`/`GatherInline`) не имел предела глубины: патологически вложенный HTML
  (вредоносный EPUB/FB2/MOBI с тысячами вложенных элементов) переполнял стек → **неперехватываемый**
  `StackOverflowException`, роняющий процесс. Достижимо через `EpubDocument.Open` (eager-пагинация на
  открытии). Добавлен предел `LayoutEngine.MaxNestingDepth = 256` (аналог `PdfCosLimits.MaxTreeDepth`):
  контент глубже предела отбрасывается, рендер не падает. Для валидного контента (никогда не вложен так
  глубоко) поведение не меняется. **5 regression-тестов** (8 000-уровневая вложенность block/inline →
  завершается без краша; мелкий контент перед глубоким — рендерится).
- **Malformed/hostile-input fuzz-корпус для EPUB/FB2/MOBI/Image** (Вектор 3, PR-3c). Расширяет PDF-корпус
  (#142) на остальные форматы: 15+ битых/вредоносных фикстур на формат (не-zip/не-XML/не-PalmDB, обрезки,
  битый контейнер/манифест, висячие spine-ссылки, 0 записей, чужой magic, глубокая вложенность) ×
  открыватели EPUB(zip)/FB2(XML)/MOBI(PalmDB)/Image(кодеки) = **41 тест**. Каждый доказывает: открытие
  битого файла **завершается быстро и либо возвращает документ, либо бросает только «ручное» исключение**
  (`InvalidDataException`/`XmlException`/`ImageFormatException`/VersOne-исключения), но **не крашит и не
  виснет**; для открывшихся документов дополнительно проверяется деградация render/text-слоя. Открыватели
  правок не потребовали — только общий `LayoutEngine` depth-guard (выше).
...
- **EPUB/HTML linked-CSS — author-каскад** (Вектор 2, PR-2d) — книги, чья типографика жила во внешнем
  ...
  **UA-defaults < author-normal (по специфичности, затем порядку) < inline-`style` < author-`!important`
  ...
  что и картинки). **+40 тестов** (36 cascade-кейсов через публичный API рендера + 4 EPUB end-to-end через
  `<link>` + резолвер).
...
- **FB2 и MOBI рендерятся визуально** (Вектор 2, PR-2c) — раньше рисовались белым холстом, теперь через
  ...
  оттестированную реализацию пагинации (без триплицирования). **FB2:** новый `Fb2HtmlConverter` (XML→HTML:
  title→h2, subtitle→h3, p, epigraph/cite→blockquote, emphasis/strong, empty-line→br; текст экранируется),
  секции flatten'ятся в главы. **MOBI:** текстовые записи (произвольные ~4 КБ срезы одного HTML-потока)
  склеиваются и режутся на главы по маркерам `<mbp:pagebreak>` (без артефактов на границах записей; fallback
  — одна глава). Загрузчики получают `IHtmlRenderer` через DI; текст-слой — текст главы на её первой
  странице. **+21 FB2 + 3 MOBI теста.** Этим Вектор 2 (визуальный рендер EPUB/FB2/MOBI) завершён;
  «белый холст»-disclaimer снят со всех форматов. Отложено: зум-reflow, попиксельный текст-слой, картинки
  FB2/MOBI.
