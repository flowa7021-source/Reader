...
Заявленные форматы (PDF, DjVu, изображения, EPUB, FB2, MOBI) **открываются** в Phase 1. **EPUB теперь рендерится визуально** — pure-managed движок `Foliant.Rendering.Html` (AngleSharp → layout → SixLabors → BGRA32, см. [`docs/HTML_RENDERER.md`](docs/HTML_RENDERER.md)) с фиксированной пагинацией по вьюпорту и встроенными картинками. **FB2 и MOBI** пока работают в режиме «read-only через текстовый слой» (страница — белый холст); их подключение к тому же движку — следующий шаг Вектора 2 (PR-2c). PDF, DjVu и изображения рендерятся полноценно. У EPUB зум-reflow и попиксельный текст-слой — последующая доработка.
