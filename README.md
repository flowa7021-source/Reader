...
Заявленные форматы (PDF, DjVu, изображения, EPUB, FB2, MOBI) **открываются** в Phase 1, но EPUB/FB2/MOBI пока работают в режиме «read-only через текстовый слой»: документ индексируется для поиска и FTS, навигация по разделам доступна, однако страница рисуется белым холстом. Полноценный визуальный рендер этих форматов — **Вектор 2** (в работе): уже заложен pure-managed движок `Foliant.Rendering.Html` (AngleSharp → layout → SixLabors → BGRA32, см. [`docs/HTML_RENDERER.md`](docs/HTML_RENDERER.md)); подключение к EPUB снимет «белый холст» (далее FB2/MOBI). PDF, DjVu и изображения рендерятся полноценно уже сейчас.
