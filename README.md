...
В рамках Phase 2 уже смёржены: цифровые подписи (просмотр + PAdES-B валидация), redaction (координатный + find-and-redact), OCG-слои (чтение + переключение видимости + переименование/удаление), watermark, колонтитулы, Bates-нумерация, merge/split/extract/insert страниц, crop, редактирование метаданных (/Info, «Document Properties» + пользовательские «Custom Properties»), метки страниц (/PageLabels, «Number Pages»), начальный вид (/ViewerPreferences, «Initial View»), вложенные файлы (/EmbeddedFiles, «Attachments»), XMP-метаданные (/Metadata), очистка от JavaScript и автодействий («Sanitize»), именованные переходы (/Names/Dests), список шрифтов с встраиванием («Fonts»), список ссылок («Links»), просмотр содержания PDF («Outlines», с режимом перехода/стилем/цветом) + экспорт закладок в PDF /Outlines с богатыми опциями, список output intents («Output Intents», PDF/X-готовность), preflight/структурную проверку PDF («Preflight» — невстроенные шрифты, JS, image-only и т. д.), открытие защищённых паролем PDF и печать (Ctrl+P). Движок устойчив к malformed/вредоносным файлам: depth-guard против StackOverflow на cos-деревьях PDF и на DOM-обходе HTML-рендера, плюс fuzz-корпус из сотен битых/враждебных фикстур по всем читателям PDF и открывателям EPUB/FB2/MOBI/Image (битый файл деградирует, не крашит и не виснет). Шифрование PDF (запись) и валидация PDF/A — пока port+stub (реальная реализация — Phase 3). Полная карта волн — в [`ROADMAP.md`](ROADMAP.md); концепт и фазы — в [`PROJECT_BOARD.md`](PROJECT_BOARD.md).

Все заявленные форматы (PDF, DjVu, изображения, EPUB, FB2, MOBI) теперь **рендерятся визуально**. EPUB/FB2/MOBI рисуются через общий pure-managed движок `Foliant.Rendering.Html` (AngleSharp → layout → SixLabors → BGRA32, см. [`docs/HTML_RENDERER.md`](docs/HTML_RENDERER.md)) с фиксированной пагинацией по вьюпорту через общий `HtmlPaginator`; EPUB поддерживает встроенные картинки и **author-CSS** (linked `<link>`-таблицы + `<style>`-блоки: настоящий каскад с селекторами/специфичностью/`!important` поверх UA-defaults, через `AngleSharp.Css`), FB2 конвертируется из XML, MOBI склеивается из записей и режется на главы по `<mbp:pagebreak>`. PDF, DjVu и изображения рендерятся нативно. Последующие доработки HTML-рендера: зум-reflow, попиксельный текст-слой (точная подсветка поиска), картинки FB2/MOBI.
