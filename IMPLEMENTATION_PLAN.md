...
| **S11** | Page management (rotate/delete/insert/reorder) | ✅ | `PdfPageEditService`/`PdfPageOps`/`PageOrder` + команды rotate/delete/move/reorder с reload; **`ThumbnailStripViewModel`** (выбор/reorder, тесты); **WPF полоса миниатюр с drag-and-drop reorder** (`MainWindow.xaml.cs` ~178–247: lazy-рендер миниатюр + DnD → `ThumbnailStripViewModel.MoveAsync`). | — |
...
| **S13** | License + триал + installer + EV-подпись | 🟡 | `EcdsaLicenseVerifier`, `TrialAntiTamperService`; **DPAPI-хранилище + реестр + marker (`DpapiLicenseStorage`/`TrialStores`), диалог импорта лицензии, dev-ключ верификатора**; **`Foliant.iss` (payload + native-модели по tier'ам + uninstall-данные) + `sign-binaries.ps1` + release-notes scaffolding**. | EV-сертификат (внешний); `foliant.ico` (дизайн); ISCC-сборка + smoke на Windows-VM. |
...
> Прим. (2026-05-26): по п.8 добавлены `EULA.md` и `PRIVACY.md`; инсталлятор показывает EULA в
> мастере и кладёт EULA + privacy + `LICENSE` + `NOTICE.md` в `{app}\Licenses\`. Тексты —
> alpha-черновики (нужна юридическая ревизия до 1.0); финальная проверка раскладки — ISCC-сборка
> на Windows.
