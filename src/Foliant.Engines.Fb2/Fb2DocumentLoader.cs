...
/// <c>FictionBook</c> в первых ~512 байтах (sniff). Загружает через
/// <see cref="Fb2Document.Open(string, IHtmlRenderer)"/> — секции конвертируются в HTML и
/// paginate/paint'ятся через <see cref="IHtmlRenderer"/>.
///
...
public sealed class Fb2DocumentLoader(ILogger<Fb2DocumentLoader> log, IHtmlRenderer renderer) : IDocumentLoader
{
    ...
    public async Task<IDocument> LoadAsync(string path, CancellationToken ct)
    {
        ...
        {
            Fb2Document doc = Fb2Document.Open(path, renderer);
