...
/// <summary>
/// Импорт form-данных из FDF — обратная операция к <see cref="FdfFormDataExporter"/>. Минимальный
/// cos-парсер scope'нут в этот файл (PdfPig свой не экспонирует, а копия из
/// <see cref="FdfAnnotationImporter"/> — файл-локальная). Достаточно для чтения
/// <c>&lt;&lt;/T &lt;name&gt; /V &lt;value&gt;&gt;&gt;</c> внутри массива <c>/Fields</c>.
///
/// Best-effort: битые элементы пропускаются; невалидный header (<c>%FDF-</c> отсутствует) бросает
/// <see cref="FormatException"/>.
/// </summary>
public sealed class FdfFormDataImporter : IFormDataImporter
{
    ...
    public IReadOnlyDictionary<string, string> Import(string content)
    {
        ...
        if (root.Get<CosDict>("FDF") is not { } fdf || fdf.Get<CosArray>("Fields") is not { } fields)
        {
            return new Dictionary<string, string>(StringComparer.Ordinal);
        }
        ...
        {
            if (item is not CosDict d)
            {
                continue;
            }

            if (d.Get<CosString>("T") is not { } nameStr)
            {
                continue;
            }

            string name = DecodePdfText(nameStr.Bytes);
            if (string.IsNullOrEmpty(name))
            {
                continue;
            }

            string value = d.Get<CosString>("V") is { } valStr ? DecodePdfText(valStr.Bytes) : string.Empty;
            result[name] = value;
        }
...
// ─── Minimal cos value model (file-local: дублирует FdfAnnotationImporter, но изолированно). ───
