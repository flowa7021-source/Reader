...
EPUB end-to-end через `<link>` + резолвер). Реальный прогон 2661→2701 (+40, 0 failed; auto-review
добавил 8 тестов: important-tie-break по специфичности, inline-`margin`-shorthand, `rgba()`/`hsla()`
alpha, `rem`/`%`-единицы). Сделано напрямую (single-assembly cascade-ядро на beta-API `AngleSharp.Css`
— инкрементальная компиляция надёжнее агентного сплита); auto-review-агент на финальном диффе
(вердикт: корректность подтверждена эмпирически, багов нет; свёрнут NIT — убран shared-mutable static).
